...
public final class ExportManifestAddressTypeAudit {
    ...
    public static void logShipperAddressTypeChange(
            ...
            Long afterShipperAddressPoid,
            String shipperAddressTypeFromRequest) {
        if (loggingService == null || addressDetailsRepository == null) {
            return;
        }
        String beforeType = resolveAddressType(addressDetailsRepository, beforeShipperAddressPoid);
        String afterType = hasText(shipperAddressTypeFromRequest)
                ? shipperAddressTypeFromRequest.trim()
                : resolveAddressType(addressDetailsRepository, afterShipperAddressPoid);
