...
/**
 * Entity class for SHIP_BL_MANIFEST_CONTAINER_DTL table
 */
@Entity
@Table(name = "SHIP_BL_MANIFEST_CONTAINER_DTL",
        uniqueConstraints = {
                @UniqueConstraint(name = "UK_SHIP_BLCNTDTL", columnNames = {"TRANSACTION_POID", "CONTAINER_NO"})
        })
@Data
@NoArgsConstructor
@AllArgsConstructor
...
public class ExportShipBlManifestContainerDtl {
    ...
    @Column(name = "EXTRA_FREE_DAYS", precision = 10, scale = 0)
    private Long extraFreeDays;

    @Column(name = "EXTRA_FREE_DAYS_PRNPLS", precision = 10, scale = 0)
    ...
    @Column(name = "TOTAL_DAYS_COLLECTED", precision = 20, scale = 3)
    private BigDecimal totalDaysCollected;

    @Column(name = "TOTAL_AMOUNT_COLLECTED", precision = 20, scale = 3)
