package com.asg.shipping.vvc.voyage.controller;
...
import com.asg.shipping.vvc.currency.dto.CurrencyUpdateRequest;
import com.asg.shipping.vvc.transhipment.dto.TranshipmentTransferRequest;
import com.asg.shipping.vvc.transhipment.dto.TranshipmentUpdateRequest;
import com.asg.shipping.vvc.voyage.dto.VoyageUpsertRequest;
import com.asg.shipping.vvc.voyage.service.VesselVoyageService;
...
public class VesselVoyageController {
    ...
    public ResponseEntity<?> get(@PathVariable Long voyagePoid) {
        log.info("Action={} | Get voyage | voyagePoid={} groupPoid={} companyPoid={} userPoid={}",
                UserContext.getActionRequested(), voyagePoid, UserContext.getGroupPoid(), UserContext.getCompanyPoid(), UserContext.getUserPoid());
    ...
    public ResponseEntity<?> create(@Valid @RequestBody VoyageUpsertRequest request) {
        log.info("Action={} | Create voyage | voyageNo={} linePoid={} vesselPoid={} groupPoid={} companyPoid={} userId={}",
                UserContext.getActionRequested(),
                request.getVoyageNo(),
                request.getLinePoid(),
    ...
    public ResponseEntity<?> update(@PathVariable Long voyagePoid, @Valid @RequestBody VoyageUpsertRequest request) {
        log.info("Action={} | Update voyage | voyagePoid={} voyageNo={} linePoid={} vesselPoid={} userId={}",
                ...
                request.getVesselPoid(),
                UserContext.getUserId());
        return ApiResponse.success("Vessel voyage updated successfully", vesselVoyageService.updateVoyage(voyagePoid, request));
    }

    @AllowedAction(UserRolesRightsEnum.VIEW)
    @GetMapping("/{voyagePoid}/bls")
    public ResponseEntity<?> listBls(
            @PathVariable Long voyagePoid,
            @RequestParam VoyageBlTab tab,
        ...
        log.info("Action={} | List BLs | voyagePoid={} tab={} filter={} page={} size={}",
                UserContext.getActionRequested(),
                voyagePoid, tab, filter, pageable.getPageNumber(), pageable.getPageSize());
        return ApiResponse.success("BLs fetched successfully", vesselVoyageService.listBls(voyagePoid, tab, filter, pageable));
    }
    ...
    public ResponseEntity<?> ediUpload(@PathVariable Long voyagePoid, @RequestPart("file") MultipartFile file) {
        log.info("Action={} | Upload EDI | voyagePoid={} fileName={} size={}",
                UserContext.getActionRequested(),
                voyagePoid,
    ...
    public ResponseEntity<?> listTranshipments(@PathVariable Long voyagePoid) {
        ...
        return ApiResponse.success("Transhipments fetched successfully", vesselVoyageService.listTranshipments(voyagePoid));
    }

    @AllowedAction(UserRolesRightsEnum.EDIT)
    @PutMapping("/{voyagePoid}/transhipments")
    public ResponseEntity<?> updateTranshipments(@PathVariable Long voyagePoid, @Valid @RequestBody TranshipmentUpdateRequest request) {
        log.info("Action={} | Update transhipments | voyagePoid={} items={}", UserContext.getActionRequested(), voyagePoid, request.getItems().size());
        return ApiResponse.success("Transhipments updated successfully", vesselVoyageService.updateTranshipments(voyagePoid, request));
    }

    @AllowedAction(UserRolesRightsEnum.EDIT)
    @PostMapping("/{voyagePoid}/transhipments/transfer")
    public ResponseEntity<?> transferTranshipments(@PathVariable Long voyagePoid, @Valid @RequestBody TranshipmentTransferRequest request) {
        ...
        return ApiResponse.success("Transhipments transfer completed", vesselVoyageService.transferTranshipments(voyagePoid, request));
    }
    ...
    public ResponseEntity<?> importHnjn(@PathVariable Long voyagePoid) {
        ...
        return ApiResponse.success("HNJN transhipments import completed", vesselVoyageService.importHnjnTranshipments(voyagePoid));
    }
    ...
    public ResponseEntity<?> listCurrency(@PathVariable Long voyagePoid) {
        ...
        return ApiResponse.success("Voyage currencies fetched successfully", vesselVoyageService.listVoyageCurrencies(voyagePoid));
    }

    @AllowedAction(UserRolesRightsEnum.EDIT)
    @PostMapping("/{voyagePoid}/currency/update")
    public ResponseEntity<?> updateCurrency(@PathVariable Long voyagePoid, @Valid @RequestBody CurrencyUpdateRequest request) {
        log.info("Action={} | Update currency | voyagePoid={} items={}", UserContext.getActionRequested(), voyagePoid, request.getItems().size());
        return ApiResponse.success("Currency update completed", vesselVoyageService.updateCurrencyRates(voyagePoid, request));
    }

    @AllowedAction(UserRolesRightsEnum.EDIT)
    @PostMapping("/{voyagePoid}/can/resend")
    public ResponseEntity<?> resendCan(@PathVariable Long voyagePoid, @RequestParam(value = "blTransactionPoid", required = false) Long blTransactionPoid) {
        log.info("Action={} | Resend CAN | voyagePoid={} blTransactionPoid={}", UserContext.getActionRequested(), voyagePoid, blTransactionPoid);
        return ApiResponse.success("CAN resend completed", vesselVoyageService.resendCan(voyagePoid, blTransactionPoid));
    }
    ...
    public ResponseEntity<?> createTdr(@PathVariable Long voyagePoid, @RequestParam(defaultValue = "true") boolean createEmptyManifestFirst) {
        log.info("Action={} | Create TDR | voyagePoid={} createEmptyManifestFirst={}",
                UserContext.getActionRequested(), voyagePoid, createEmptyManifestFirst);
    ...
    public ResponseEntity<?> exportCosco(@PathVariable Long voyagePoid, @RequestParam(value = "blPoid", required = false) Long blPoid) {
        log.info("Action={} | Export COSCO EDI | voyagePoid={} blPoid={}", UserContext.getActionRequested(), voyagePoid, blPoid);
    ...
    public ResponseEntity<?> importGeneralCargo(@PathVariable Long voyagePoid) {
        ...
        return ApiResponse.success("General cargo import completed", vesselVoyageService.importGeneralCargo(voyagePoid));
    }
    ...
    public ResponseEntity<?> downloadExcel(@PathVariable Long voyagePoid, @PathVariable String type) {
        log.info("Action={} | Download Excel export | voyagePoid={} type={}", UserContext.getActionRequested(), voyagePoid, type);
    ...
    public ResponseEntity<?> downloadManifest(
            @PathVariable Long voyagePoid,
