...
public interface ShipLineTariffImpDtlRepository extends JpaRepository<ShipLineTariffImpDtl, ShipLineTariffImpDtlId> {
    ...
    @Modifying
    @Query(value = """
            INSERT INTO PRODUCTION.SHIP_LINE_TARIFF_IMP_DTL (TRANSACTION_POID, DET_ROW_ID, CONTAINER_TYPE_POID)
            SELECT :transactionPoid,
                   (SELECT COALESCE(MAX(det_row_id), 0) FROM PRODUCTION.SHIP_LINE_TARIFF_IMP_DTL WHERE TRANSACTION_POID = :transactionPoid) + ROWNUM,
                   CONTAINER_TYPE_POID
    ...
    @Modifying
    @Query(value = "DELETE FROM PRODUCTION.SHIP_LINE_TARIFF_IMP_DTL WHERE TRANSACTION_POID = :transactionPoid AND DET_ROW_ID IN (:detRowIds)", nativeQuery = true)
